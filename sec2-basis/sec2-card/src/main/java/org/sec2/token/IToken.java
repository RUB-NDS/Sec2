...
public interface IToken {
    ...
    /**
     * Encrypts a document-key with a group-key, which has to be stored on the
     * card already. The group-key is referenced by an id. An all-zero IV is
     * used for the CBC mode. This function requires authentication with a PIN.
     *
     * @param keyId id of the group-key to use for encryption
    ...
    /**
     * Encrypts a document-key with a group-key, which has to be stored on the
     * card already. The group-key is referenced by an id. This function
    ...
    /**
     * Decrypts a document-key with a group-key. Uses an all-zero IV for the
     * CBC mode. Requires PIN authentication.
     *
     * @param keyId id of the group-key to use, must exist on token
    ...
    /**
     * Remove a group-key from the card. Requires PIN authentication.
     *
     * @param keyId id of group-key to remove
     * @throws TokenException
     */
    public void removeGroupKey(GroupKeyId keyId) throws TokenException;

    /**
     * Import a groupKey into the token's keystore. The group-key is
     * encrypted and thus wrapped for the user's private key. Requires PIN
     * authentication.
     *
     * @param groupKey group-key to be imported
     * @throws TokenException
     */
    public void importGroupKey(GroupKey groupKey) throws TokenException;

    /**
     * Check if a given group-key is already in the card. Requires PIN
     * authentication.
     *
     * @param keyId id of the group-key to find in the token's keystore.
    ...
    /**
     * Get a list of all available group-keys stored in the token's keystore.
     * Requires PIN authentication.
     *
     * @return list of group-key ids
     * @throws TokenException
     */
     ...
     * "forget" all previously imported group-keys. Requires PIN
     * authentication.
     *
     * @throws TokenException
     */
