...
public class TokenException extends Exception {
    ...
    public TokenException(int sw) {
        ...
        switch (sw) {
            case ReturnCodes.SW_GKEY_NOT_SET:
                exceptionMsg += "GroupKey is not set";
                break;
            case ReturnCodes.SW_GKEY_WRONG_ID_SIZE:
                exceptionMsg += "GroupKey ID has wrong size";
                break;
            case ReturnCodes.SW_GKEY_WRONG_SIZE:
                exceptionMsg += "GroupKey has wrong size";
                break;
            case ReturnCodes.SW_GKEY_SLOT_COUNT_INVALID:
                exceptionMsg += "Slot count for GroupKeyStore is invalid";
                break;
            case ReturnCodes.SW_GKEY_SLOT_EMPTY:
                exceptionMsg += "GroupKey slot is empty";
                break;
            case ReturnCodes.SW_GKEY_SLOT_INVALID:
                exceptionMsg += "GroupKey slot is invalid";
                break;
            case ReturnCodes.SW_GKEY_SLOT_IN_USE:
                exceptionMsg += "GroupKey slot is in use";
                break;
            case ReturnCodes.SW_GKEY_DATA_INVALID:
                exceptionMsg += "Could not encrypt/decrypt with GroupKey: data invalid (e.g., too short, IV missing, etc.)";
                break;
