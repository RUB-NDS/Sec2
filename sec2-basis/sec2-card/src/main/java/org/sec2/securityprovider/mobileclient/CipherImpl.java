...
import static org.sec2.securityprovider.serviceparameter.TokenType.HARDWARE_TOKEN;
...
final class CipherImpl extends CipherSpi implements IServiceImpl {
      ...
      throws NoSuchPaddingException {
        ...
        try {
            ...
            throw new NoSuchPaddingException(
              "Padding not supported by crypto library: " + padding);
        }
    }
    ...
    protected void engineInit(final int opmode, final Key key,
      ...
      throws InvalidKeyException {
        ...
        // random is not used
        // check key
        if (key == null || !(key instanceof GroupKey)) {
            throw new InvalidKeyException("Key must be of NON NULL type"
              + " and GroupKey");
        }

        this.groupKey = (GroupKey) key;
    }
                ...
                switch (mode) {
                    case Cipher.WRAP_MODE:
                        wrappedKey = tokenInterface.encryptDocumentKey(
                          this.groupKey.getId(), deliveredKey);
                        break;
                    case Cipher.UNWRAP_MODE:
