...
/**
 * Key specification for group/document key creation.
 *
...
public final class Sec2SecretKeySpec implements KeySpec {
    ...
    /**
     * Key type - only Document or Group key type are valid.
     */
      ...
      final KeyType type) {
        ...
        if (!(algorithm.name().equals(CipherAlgorithm.AES.name()))) {
            throw new IllegalArgumentException("Only AES is possible for"
              + " Group and Document- Keys");
        }
        this.cipherAlgorithm = algorithm;

        if (type == null
          || (type != KeyType.GROUP && type != KeyType.DOCUMENT)) {
            throw new IllegalArgumentException(
              "Only Group or Document key types are allowed.");
        }
        this.keyType = type;
    }
