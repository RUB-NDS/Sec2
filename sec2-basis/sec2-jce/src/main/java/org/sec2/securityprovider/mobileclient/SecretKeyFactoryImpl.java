...
final class SecretKeyFactoryImpl extends SecretKeyFactorySpi
    ...
    protected KeySpec engineGetKeySpec(final SecretKey key, final Class keySpec)
        ...
        } else if (key instanceof ClusterKey) {
            ClusterKey ckey = (ClusterKey) key;
            length = ckey.getKey().getBytes().length;
            algo = CipherAlgorithm.valueOf(ckey.getAlgorithm());
            type = KeyType.CLUSTER;
        } else {
            throw new InvalidKeySpecException(
